...
class Adafruit_Thermal:
    ...
    def __init__(self, bus=1, baudrate=9600, pins=None, **kwargs):
        ...
        self.uart = UART(0, baudrate=9600, stop=1, rx=Pin(1, Pin.IN), tx=Pin(0, Pin.OUT))
        ...
        print("Set print defaults")
        # TODO CGMORSE - Adds a "7x("
#         self.writeBytes(
#           27,           # Esc
#           55,           # 7 (print settings)
#           heatDots,     # Heat dots
...
#         self.dotFeedTime  = 2
    ...
    def timeoutSet(self, x):
        self.resumeTime = self.ticks_add(int(utime.ticks_ms()), int(x))
    ...
    def begin(self, heatTime=defaultHeatTime):
        #self.write(f"\x1Bd\x37d\xd{bytes(heattime)\x28d}")
        self.writeBytes(
          27,       # Esc
          55,       # 7 (print settings)
          11,       # Heat dots
    ...
    def feed(self, x=1):
        #self.write(f"\x1Bd{chr(x)}")
        self.writeBytes(27, 100, x)
        print(self.dotFeedTime * self.charHeight)
        self.timeoutSet(round(self.dotFeedTime * self.charHeight))
        self.prevByte = '\n'
        self.column   =    0
